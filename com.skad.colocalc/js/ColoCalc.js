...
Coloc.prototype.start = function() {
  ...
  let users = this.fs.readdirSync(this.users_dir).map(o => {
    return o.split("_")[0];
  }).filter(
    (elem, index, self) => {
  ...
  users.forEach(o => {
    $("#user").append("<option value='" + o + "' >" + o + '</option>');
  });
...
Coloc.prototype.buildPdf = function(callback = null) {
  ...
  let Globalrows = obj.users.map((e) => {
    return [e, obj.credits[e]];
  });
...
Coloc.prototype.resetCredit = function() {
  ...
  $(inst.database).each(function() {
    let elem = this.users;
    $(inst.users).each(function() {
      elem[this].realPayed = elem[this].part;
    });
...
Coloc.prototype.bindAddForm = function() {
  ...
  $(modal).find(".payed").unbind("change").change(function() {
    ...
    part = $(modal).find("#total").val() / nbUsers;
    part = part.toFixed(2);
    $(modal).find(".payed").each(function() {
      $(modal).find("#" + this.id + ".realPayed").val(0)
    });
    $(modal).find(".payed:checked").each(function() {
      $(modal).find("#" + this.id + ".realPayed").val(part)
    });
  ...
  $(modal).find("#total").unbind("change").change(function() {
    ...
    part = $(modal).find("#total").val() / nbUsers;
    part = part.toFixed(2);
    $(modal).find(".participate:checked").each(function() {
      $(modal).find("#" + this.id + "_realPayed").val(part)
    });
  });
...
Coloc.prototype.bindConfigForm = function() {
  ...
  $(modal).find("#save").unbind("click").click(function() {
    ...
    $(modal).find(".user").each(function() {
      inst.users.push($(this).val());
    });
    inst.types = Array();
    $(modal).find(".type").each(function() {
      inst.types.push($(this).val());
    });
...
Coloc.prototype.BuildInfo = function(id) {
  ...
  $(this.users).each(function() {
    ...
    if (!d.users[this] || (d.users[this] && !d.users[this].participate)) {} else {
...
Coloc.prototype.BindInfo = function(id) {
  ...
  $(this.modal).find("#close").unbind().bind("click", () => {
    coloc.closeModal();
  });
};
